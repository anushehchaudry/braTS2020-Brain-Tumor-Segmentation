...
def majority_voting_for_one_patient_with_prob_comparison(patient_name, models):
    ...
    probs_cur_patient = []
    for model in models:
        result_path = os.path.join("/content/drive/Shareddrives/cs231n_final/BiTr-Unet-main/postprocess/output/submission/", model, patient_name)
...
def ensemble_majority_voting(models):
    ...
    # get patients name
    pred_path = os.path.join("/content/drive/Shareddrives/cs231n_final/BiTr-Unet-main/postprocess/output/submission/", models[0])
...
def averaging_for_one_patient(patient_name, models, threshold=0.5):
    probs_cur_patient = []
    for model in models:
        prob_path = os.path.join("/content/drive/Shareddrives/cs231n_final/BiTr-Unet-main/postprocess/output/submission/", model + "_probabilityMap", patient_name + ".npy")
        # read predicted prop
        prob_cur_patient = np.load(prob_path)
...
if __name__ == "__main__":
    ...
    model_list = []
    model_list.append("BiTrUnet20232023-06-07")
    model_list.append("BiTrUnet20232023-06-07")
    ...
    test_path = '/content/drive/Shareddrives/cs231n_final/cs231n_baseline/BraTS2020/BraTS2020_ValidationData/MICCAI_BraTS2020_ValidationData'

    if method == 0:
        prediction_dir = os.path.join("/content/drive/Shareddrives/cs231n_final/BiTr-Unet-main/postprocess/", "ensembleBITRANS_4".format(len(model_list)))
        ensemble_majority_voting(model_list)

    elif method == 1:
        prediction_dir = os.path.join("/content/drive/Shareddrives/cs231n_final/BiTr-Unet-main/postprocess/output", "ensemble_{}_[214]averaging".format(len(model_list)))
